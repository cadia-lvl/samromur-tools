...
def modify_subset(master_file, subset_file):
    ...
    # Read both files to a dataframe.
    ...
    for id in subset_ids_padded:
        ...
        # The value at 'released' should always be v1. This check is just a one-time safeguard and should be deleted as soon as possible.
        if subset_df.at[id.lstrip('0'), 'released'] != 'v1':
            print(f'Invalid \'released\' value detected: {subset_df.at[id.lstrip("0"), "released"]}')
            return

        # r1 is more descriptive than v1
        master_df.at[id, 'released'] = 'r1'

        # TODO: For each filename, change .wav to .flac

        # This copies the old value of 'released' to the new metadata file. The old value is v1 for all rows, we don't wan that. It should be safe to delete this line entirely along with this comment.
