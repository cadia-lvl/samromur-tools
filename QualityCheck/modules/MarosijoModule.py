...
class MarosijoTask():
    ...
    Librispeech: An ASR corpus based on public domain audio books. In ICASSP, IEEE International 
    Conference on Acoustics, Speech and Signal Processing - Proceedings (Vol. 2015-August, pp. 5206–5210). 
    http://doi.org/10.1109/ICASSP.2015.7178964
    """

    def __init__(self, modelPath, u_prefix, downsample=False ):
    ...
    def common(self):
        ...
        if not hasattr(self, '_common'):
            self._common = MarosijoCommon(downsample=self.downsample, modelPath = self.modelPath, u_prefix= self.u_prefix)
        return self._common
