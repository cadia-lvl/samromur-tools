...
path_to_previous_metadata_file = '/home/derik/work/samromur_validation/samromur_wip/metadata_total_041220.tsv'
...
for i in tqdm(df.index, unit='lines'):
        ...
        text.append([i,s])
        split_data.append[i,df.at[i, 'speaker_id'],s ]
        for tok in s.split(' '):
            tokens.add(tok)
...
#status = split_train_test_eval(split_data, precentage_test, precentage_eval)
#for i in tqdm(df.index, unit='lines'):
